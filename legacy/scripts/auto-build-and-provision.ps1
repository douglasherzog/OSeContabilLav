...
if (-not (Test-Path $nodeExe)) {
  ...
  try {
    ...
    # Bypass temporário de TLS (ambiente corporativo)
...
$npmShimContent = "@echo off`r`nsetlocal`r`nset \"NODE_EXE=$nodeExe\"`r`nset \"NPM_CLI=$npmCli\"`r`n\"%NODE_EXE%\" \"%NPM_CLI%\" %*`r`nendlocal`r`n"
