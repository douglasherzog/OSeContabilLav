...
export default function ContasPagar() {
  ...
  const [payForm, setPayForm] = useState({ paid_at: today(), method: 'pix', account_label: '' });
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { setRows(await window.api.ap.list(filters) || []); }
    finally { setLoading(false); }
  }, [filters]);

  const loadCategories = useCallback(async () => {
    try { setCategories(await window.api.ap.categories.list() || []); }
    catch { }
  }, []);
  ...
  async function handlePay() {
    try {
      ...
      await window.api.ap.update(payload);
  ...
  async function handleUnpay(r) {
    await window.api.ap.update({ ...r, status: 'pendente', paid_at: null, method: null });
    load();
  }

  async function handleDelete(id) {
    if (!confirm('Excluir conta?')) return;
    try { await window.api.ap.delete(id); toast.success('Conta excluída.'); load(); }
  ...
  async function handleDeleteCategory(name) {
    ...
    try { await window.api.ap.categories.delete(name); toast.success('Categoria removida.'); loadCategories(); }
    ...
    <div className="p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <button onClick={() => exportCSV(rows, `ContasPagar_${today()}.csv`)} className="flex items-center gap-1.5 border border-gray-200 text-gray-600 px-3 py-1.5 rounded-lg text-sm hover:bg-gray-50" disabled={rows.length === 0}>
      ...
      <div className="bg-white rounded-xl border p-3 mb-4 flex items-center justify-between">
        <div className="text-sm text-gray-500">{rows.length} conta(s) • {filters.status === 'pendente' ? 'pendentes' : filters.status === 'paga' ? 'pagas' : 'todas'}</div>
            ...
            {loading ? <tr><td colSpan={7} className="text-center py-8 text-gray-400">Carregando...</td></tr>
              ...
              : rows.map(r => (
                <tr key={r.id} className="border-b last:border-0 hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2.5">
                    {r.status === 'paga'
                      ? <span className="text-xs px-2 py-0.5 rounded-full bg-green-100 text-green-700 font-medium">paga</span>
                      : isOverdue(r)
                  ...
                  <td className="px-3 py-2.5 text-right font-medium">R$ {brl(r.amount)}</td>
                  <td className="px-3 py-2.5">
                    <div className="flex gap-1 justify-end">
                      {r.status !== 'paga'
                        ? <button onClick={() => { setPayModal(r); setPayForm({ paid_at: today(), method: 'pix', account_label: '' }); }} className="p-1 text-gray-400 hover:text-green-600" title="Marcar como paga"><CheckCircle size={15} /></button>
                        : <button onClick={() => handleUnpay(r)} className="p-1 text-gray-400 hover:text-yellow-600" title="Reabrir"><XCircle size={15} /></button>
                      }
        ...
        <div className="space-y-3">
              ...
              <div className="flex gap-2">
                <div className="flex-1 flex items-center gap-1">
                  <select className="w-full border rounded-lg px-3 py-2 text-sm" value={payForm.method} onChange={e => setPayForm(f => ({ ...f, method: e.target.value }))}>
                ...
                <div className="flex-1 flex items-center gap-1">
                  <select className="w-full border rounded-lg px-3 py-2 text-sm disabled:opacity-50" value={payForm.account_label} onChange={e => setPayForm(f => ({ ...f, account_label: e.target.value }))} disabled={payForm.method==='dinheiro'}>
